...
//只能在一个类文件中引用,2个一起引用会出错

#ifndef LearnLearnLearn_WWDContext_h
#define LearnLearnLearn_WWDContext_h

/*该方法负责绘制圆角矩形.x1,y1:是圆角矩形左上角的坐标;width,height:控制圆角矩形的宽,高
...
void CGContextAddRoundRect(CGContextRef c, CGFloat x1, CGFloat y1, CGFloat width, CGFloat height, CGFloat radius){
    ...
    CGContextAddLineToPoint(c, x1 + width - radius, y1); //添加一段连接到右上角的线段
    //添加一段圆弧
...
/*
 该方法负责绘制花朵
 n:控制花朵的瓣数; dx,dy控制花朵的位置; size:控制花朵的大小
 length:控制花瓣的长度
 */
void CGContextAddFlower(CGContextRef ctx, NSInteger n, CGFloat dx, CGFloat dy, CGFloat size, CGFloat length){
    CGContextMoveToPoint(ctx, dx, dy + size);
    ...
    for (int i = 1; i <= n; i++) {
        ...
        //添加二级曲线路径
        CGContextAddQuadCurveToPoint(ctx, ctrlX, ctrlY, x, y);
    }
}
